// Time BAR fuctions and data
//change the file url to match yours
$.getJSON("https://airguardianfgs.github.io/test.geojson").done(function (
  data
) {
...
function processData(data) {
    ...
    for (var attribute in properties) {
      if (
        ...
        if ($.inArray(attribute, timestamps) === -1) {
          // JQuery in.Array() method searches for a specified value within an array and return its index (or -1 if not found)
  ...
  return {
    // the function finally returns the timestamps array, the min and max of population data
  ...
  cities = L.geoJson(data, {
    ...
    pointToLayer: function (feature, latlng) {
      return L.circleMarker(latlng, {
        // we use circle marker for the points
...
function updatePropSymbols(timestamp) {
  cities.eachLayer(function (layer) {
    // eachLayer() is an Leaflet function to iterate over the layers/points of the map
...
function createSliderUI(timestamps) {
  var sliderControl = L.control({ position: "bottomright" }); // position of the slider
...
function createTimeLabel(startTimestamp) {
  var temporalLegend = L.control({ position: "bottomright" }); // same position as the slider
